...
START_TEST(test_chacha20_poly1305_compat)
{
	proposal_t *self, *other, *selected;

	self = proposal_create_from_string(PROTO_IKE, "chacha20poly1305-prfsha256-ecp256");
	other = proposal_create(PROTO_IKE, 0);
	other->add_algorithm(other, ENCRYPTION_ALGORITHM, ENCR_CHACHA20_POLY1305, 256);
	other->add_algorithm(other, PSEUDO_RANDOM_FUNCTION, PRF_HMAC_SHA2_256, 0);
	other->add_algorithm(other, DIFFIE_HELLMAN_GROUP, ECP_256_BIT, 0);

	selected = self->select(self, other, TRUE, FALSE);
	ck_assert(selected);
	assert_proposal_eq(selected, "IKE:CHACHA20_POLY1305_256/PRF_HMAC_SHA2_256/ECP_256");
	selected->destroy(selected);
	selected = self->select(self, other, FALSE, FALSE);
	assert_proposal_eq(selected, "IKE:CHACHA20_POLY1305_256/PRF_HMAC_SHA2_256/ECP_256");
	ck_assert(selected);
...
Suite *proposal_suite_create()
{
	...
	tcase_add_test(tc, test_chacha20_poly1305_compat);
	suite_add_tcase(s, tc);
